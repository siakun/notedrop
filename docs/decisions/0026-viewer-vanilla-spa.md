...
- **Supersedes**: -
- **Superseded by**: spec §5.2 + §11.1.2 + ADR-0011 (표준 복귀)
...
- viewer 재작성은 본 세션에서 진행. spec §5.2 + §5.4 + §11.1.2 + ADR-0011 (Static SPA: Next.js `output: 'export'` + `'use client'`) + ADR-0012 (Hexagonal + unified.js) 가 표준. 기존 vanilla 코드는 `backup-vanilla-viewer` 브랜치에 보존.
