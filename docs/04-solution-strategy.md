...
이유: 단위 테스트가 옵시디언 없이 가능. *요구사항 변경 시 몇 곳을 고치는가* 가 일관되게 적음 (Adapters 만 교체하면 됨). 미래에 옵시디언 외 다른 vault 형식 지원 시 Adapter 추가만으로 가능.
...
| 뷰어 UI UX 개선 (폰트, 색, 레이아웃) | 뷰어만 (콘텐츠 재발행 X - 자동 재배포만) |
